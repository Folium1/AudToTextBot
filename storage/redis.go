...
func (r *RedisStorage) SavePremiumUser(userId int) error {
	...
	err := r.db.Expire(ctx, fmt.Sprintf("tg:time:premium:%v", userId), monthDuration).Err()
	if err != nil {
		return err
	}
	return err
...
func (r *RedisStorage) SaveUnpremiumUser(userId int) error {
	...
	err = r.db.Expire(ctx, fmt.Sprintf("tg:time:unpremium:%v", userId), monthDuration).Err()
	if err != nil {
		return err
	}
	return nil
